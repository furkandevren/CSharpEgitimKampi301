...
namespace CSharpEgitimKampi301.EFProject
{
    public partial class FrmStatistics : Form
    {
        ...
        private void FrmStatistics_Load(object sender, EventArgs e)
        {
            ...
            lblRomeGuidName.Text = db.Guide.Where(x => x.GuideId == romeGuideId).Select(y => y.GuideName + " " + y.GuideSurname).FirstOrDefault();
            ...
            var guideIdByNameFurkanDevren=db.Guide.Where(x=>x.GuideName=="Furkan"&& x.GuideSurname=="Devren").Select(y=>y.GuideId).FirstOrDefault();
            lblFurkanDevrenLocationCount.Text = db.Location.Where(x => x.GuideId == guideIdByNameFurkanDevren).Count().ToString();
        }
